...
@njit(fastmath=True)
def superior_zero(arr):
    return np.any(arr[:, -1] > 0)
...
class Object3D:
    ...
    def screen_projection(self):
        ...
        vertices[(vertices > 2) | (vertices < -2)] = 0 # Screen Clipping
        ...
        for index, color_face in enumerate(self.color_faces):
            ...
            if any(i >= len(vertices) for i in face):
                continue  # Skip faces with out-of-bounds vertices

            polygon = vertices[face]
            if not any_func(polygon, self.render.H_WIDTH, self.render.H_HEIGHT) and superior_zero(polygon):
